...
def plot_all_dataframes(dataframes: dict, title_suffix=None, xlabel=None, axs=None,
                        fields=None, nolabel=False, label='', **kwargs):
    ...
    If `fields` is provided, it must contain keys in `dataframes` and specifies
    the order in which to plot them. If not provided, `dataframes.keys()` is
    used.
    ...
    if fields is None:
        fields = dataframes.keys()

    for ax, field in zip(axs, fields):
        ax.set_prop_cycle(None)

        for name, series in dataframes[field].iteritems():
            if nolabel:
                augmented_label = ''
...
def plot_averaged_training_charts(
    ...
    plot_all_dataframes(averages, title_suffix=get_title_string(specs), xlabel=xlabel, axs=axs,
                        fields=fields, nolabel=nolabel, linewidth=linewidth, **plot_kwargs)
    ...
    for extra in extra_lines:
        ...
        for reduce_fn in reduce_fns:
            ...
            plot_all_dataframes(reduced, axs=axs, fields=fields, nolabel=True,
                                linewidth=linewidth * thin_factor, **plot_kwargs)
